...
import { useAuthStore } from '@/store/auth.store';
import { Layout } from '@/layout/Layout';
import { getLeaveBalance, processLeaveAction } from '@/services/LeaveWorkflowService';

const workflowSteps = ['Submitted', 'HOD Review', 'HR/Admin Review', 'Completed'];
...
const ApplicationDetails = () => {
  ...
  const employee = application ? users.find((u) => u.id === application.employeeId) : null;
  const leaveType = application ? leaveTypes.find((lt) => lt.id === application.leaveTypeId) : null;
  const balance = application ? getLeaveBalance(application.employeeId, application.leaveTypeId) : null;
  ...
  const canEndorse = currentUser?.role === 'hod' && application.status === 'pending_hod';
  const canFinalProcess =
    (currentUser?.role === 'admin' || currentUser?.role === 'hr') && application.status === 'pending_admin';

  const getCurrentStep = () => {
    if (application.status === 'pending_hod') return 1;
    if (application.status === 'pending_admin') return 2;
    return 3;
  };
  ...
  const getWorkflowTimeline = () => [
    {
      ...
      status:
        application.status === 'pending_hod'
          ? 'pending'
          : application.status === 'rejected' && application.hodComment
          ? 'rejected'
          : 'endorsed',
      comment: application.hodComment,
    },
    {
      label: 'HR/Admin Review',
      status:
        application.status === 'pending_hod'
          ? 'pending'
  ...
  const handleApprove = () => {
    ...
    const action = currentUser.role === 'hod' ? 'endorse' : 'approve';

    const result = processLeaveAction(application.id, action, currentUser, { comments });
    ...
    toast({
      title: currentUser.role === 'hod' ? 'Application Endorsed' : 'Application Approved',
      description: result.message,
    });

    setApproveDialogOpen(false);
    setComments('');
    navigate('/leave-workflow');
  };
  ...
  const handleReject = () => {
    ...
    const result = processLeaveAction(application.id, 'reject', currentUser, { comments });
    ...
    <Layout>
      <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Reference: LV-{new Date(application.appliedDate).getFullYear()}-{application.id.padStart(3, '0')}
              </p>
            </div>
          </div>
          <Badge variant="outline" className={getStatusColor(application.status)}>
        ...
        <div
          ...
          style={{ backgroundColor: leaveType.color + '15', borderLeft: `4px solid ${leaveType.color}` }}
        >
          <div>
            <p className="text-xs text-muted-foreground font-medium">Leave Type</p>
            <p className="text-lg font-bold" style={{ color: leaveType.color }}>
              {leaveType.name}
            </p>
          </div>
          <div className="text-right">
        ...
        <div className="bg-card rounded-xl border border-border p-5 space-y-2">
          ...
          <div className="grid grid-cols-4 gap-3">
            <div className="rounded-lg bg-muted/50 p-3">
              ...
              <p className="font-semibold">{balance?.total ?? 0}</p>
            </div>
            <div className="rounded-lg bg-muted/50 p-3">
              <p className="text-xs text-muted-foreground">Used</p>
              <p className="font-semibold">{balance?.used ?? 0}</p>
            </div>
            <div className="rounded-lg bg-muted/50 p-3">
            ...
            <div className="rounded-lg bg-info/10 p-3 border border-info/20">
              <p className="text-xs text-info">Available</p>
              <p className="font-semibold text-info">{available}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {(canEndorse || canFinalProcess) && (
            <>
              <Button
                variant="outline"
                size="sm"
                className="gap-2 text-destructive border-destructive/20 hover:bg-destructive/10 ml-auto"
              ...
              <Button size="sm" className="gap-2" onClick={() => setApproveDialogOpen(true)}>
                <CheckCircle className="w-4 h-4" />
                {canEndorse ? 'Endorse' : 'Verify & Approve'}
              </Button>
            </>
          )}
        </div>
      ...
      <Dialog open={approveDialogOpen} onOpenChange={setApproveDialogOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{canEndorse ? 'Endorse Application' : 'Verify & Approve Application'}</DialogTitle>
            <DialogDescription>
              {canEndorse
                ? 'This application will be forwarded to HR/Admin for final processing.'
                : 'This will complete verification, approve the leave, and automatically update the leave balance.'}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <WorkflowStepper currentStep={getCurrentStep()} />
            <Separator />
            <div className="space-y-1 text-sm">
              <p>
                <span className="font-semibold">Applicant:</span> {employee.name}
              </p>
              <p>
                <span className="font-semibold">Leave Type:</span> {leaveType.name}
              </p>
              <p>
                <span className="font-semibold">Duration:</span> {application.days} days
              </p>
              {!canEndorse && (
                <p>
                  <span className="font-semibold">Available Balance:</span> {available} days
                </p>
              )}
            </div>
            ...
            {!canEndorse && (
              <div className="p-3 bg-success/10 rounded-lg border border-success/20">
                <p className="text-sm text-success">
                  ✓ On approval, pending leave will be moved automatically to used leave.
                </p>
              </div>
            )}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleApprove}>{canEndorse ? 'Confirm Endorsement' : 'Confirm Approval'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                placeholder="Add your rejection reason..."
                value={comments}
                onChange={(e) => setComments(e.target.value)}
                rows={3}
              />
